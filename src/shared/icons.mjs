/**
 * 内联图标集：iconify 的 Lucide 集合（24×24、线性描边、`currentColor`）。
 *
 * 站点与油猴面板共用。内联 SVG 而非运行时请求 CDN，才能在 Shadow DOM 与离线
 * 环境下正常工作，也避免额外网络请求。界面一律使用图标，不使用 emoji。
 * @module shared/icons
 */

/**
 * 图标路径表。值即 `<svg>` 的子节点（Lucide 原始几何）。
...
const ICONS = {
	...
	auto: '<path d="M18 5h4"/><path d="M20 3v4"/><path d="M20.985 12.486a9 9 0 1 1-9.473-9.472c.405-.022.617.46.402.803a6 6 0 0 0 8.268 8.268c.344-.215.825-.004.803.401"/>',
...
/**
 * 生成一个内联 SVG 图标。
 * @param {string} name 图标名（见 {@link ICONS}）
 * @param {{ size?: number; class?: string }} [options] 选项
...
export function icon(name, options = {}) {
	...
	return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="${size}" height="${size}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"${className}>${body}</svg>`
}
